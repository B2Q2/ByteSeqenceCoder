...
if __name__ == "__main__":
    bmp = ByteSequenceCoder(fileformat="bmp")
    bitmap = ArrayPixel(4,4)
    bitmap.addDWORD()

    bmp.setByteCell('bffailSize', 54+bitmap.getBytesSize(), 1) #54 + bitmap.getBytesSize()
    bmp.setByteCell('biWidth', bitmap.width, 1)
    bmp.setByteCell('biHeight', bitmap.hight, 1)
    bmp.setByteCell('biSizeImage', bitmap.getBytesSize(), 1)
    bmp.setByteCell('arrayColor', bitmap.arr)
    #print(bmp)
    bmp.saveFile(title="testBMP")
    print(bmp)
    bmp.loadFile(title="testBMP")
    print(bmp)
